...
import pyperclip # pip install pyperclip88
...
def CookieScrapper():
    ...
    sleep(5)
    pyautogui.click(x=1741,y=69)
    sleep(2)
    pyautogui.click(x=1543,y=267)
    sleep(2)
    pyautogui.click(x=1439,y=124)
    sleep(2)
...
def MainExecution():
    ...
    while True:
        try:
               ...
               RealQustion = (Query)
               results = bard.get_answer(RealQustion)['content']
               current_datetime = datetime.datetime.now()
               #formatted_time = current_datetime.strftime("%H%M%S")
               #filenamedate = str(formatted_time) + str(".txt")
               #filenamedate = "Brain\\DataBase\\" + filenamedate
               print(split_and_save_paragraphs(results, RealQustion))
               text = RealQustion
               text = results
               #RealQustion(speak(text))
               #RealQustion = speak
               speak(text)
